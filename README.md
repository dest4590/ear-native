...
- `src/main.rs`: application state, UI composition, command dispatch, and packet handling
- `src/bluetooth.rs`: Linux and Windows Bluetooth backends
- `src/models.rs`: model metadata and SKU-to-model mapping
- `src/protocol.rs`: packet encoding/decoding and protocol command constants
- `src/ui.rs`: shared `iced` colors and button styles
- `res/assets/`: product images used by the UI
